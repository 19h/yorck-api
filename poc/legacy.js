...
const RFOO = movies => ({
	cmx () {
		...
		movies.map(movie => {
			...
			movie.program.forEach(entry => {
				const realName = cref.name.get(entry[0]);

				sc[realName] = sc[realName] || {
					movies: [],
					geometry: lRef[entry[0]]
				};
	...
	cinemas () {
		return _.keyBy(Object.keys(lRef).map(k => lRef[k]), 'name');
	}
});
